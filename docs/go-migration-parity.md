...
| QEMU/QMP ownership (`qemu.py`, `qmp.py`) | `internal/qemu` | Process-group lifecycle, bounded stop/kill, QMP greeting/capabilities/commands/errors | `test_qemu.py`, `test_qmp.py` | `qmp_test.go` synthetic-peer lifecycle and failure tests | Partial: synchronous QMP client complete; process controller not started; Go rejects QMP messages over 1 MiB |
