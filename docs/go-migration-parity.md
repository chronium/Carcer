...
| Feature ledger (`feature_requests.py`) | `internal/store` | Sparse monotonic IDs, UTF-8 byte bounds, atomic durable decisions, gate-only approval/denial | `test_feature_requests.py`, feature guest/console tests | — | Not started |
...
| Planning evidence (`planning_evidence.py`) | `internal/provenance` | Private text, public digest/identity only, attempts and interruption/failure durability | `test_planning_evidence.py`, generation worker tests | — | Not started |
