...
| App-server transport (`codex_app_server.py`) | `internal/codexapp` | JSONL process protocol, catalog validation, notifications, interrupts, errors, bounded stderr and shutdown | generation/review worker tests, fake app server | `token_usage_test.go`, `token_usage_conformance_test.go` | Partial: cumulative token snapshots and exact non-duplicating deltas complete; process ownership and JSONL routing not started |
