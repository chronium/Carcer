...
| Serial connection and READY (`serial.py`, `guest_startup.py`) | `internal/guest` | Unix connection deadlines, bounded diagnostics, canonical READY marker | `test_serial.py`, `test_candidate_boot.py`, `test_seed_boot.py` | `serial_test.go` raw transport and diagnostic tests | Partial: connect/read/write/close and diagnostic escaping complete; duplex pump and dispatcher-owned READY remain |
