...
| App-server transport (`codex_app_server.py`) | `internal/codexapp` | JSONL process protocol, catalog validation, notifications, interrupts, errors, bounded stderr and shutdown | generation/review worker tests, fake app server | message/token unit, fuzz, and Python conformance tests | Partial: bounded UTF-8 JSONL codec and cumulative token deltas complete; process ownership and concurrent routing not started |
