...
/* Private handle tables: no inheritance or transfer; tokens never recur. */
struct open_file {u64 token,identity,pos;u32 flags;};
static struct open_file handles[NT][FHC];
static u64 handle_serial;
static void file_cleanup(u32 owner) {
    for(u32 i=0;i<FHC;i++)if(handles[owner][i].token) {
        (void)fdrop(handles[owner][i].identity);
        handles[owner][i].token=0;
    }
}
