...
static int path(struct bytes b){return file_path_valid(b.d,b.n);}
...
case 5:{if(v.count)break;u64 f=lock();tool_build(id);unlock(f);return;}
case 6:{if(v.count!=1||v.a[0].n>FINISH_GENERATION_HANDOFF_MAX||!file_utf8(v.a[0].d,v.a[0].n))break;u64 f=lock();tool_finish(id,v.a[0].d,v.a[0].n);unlock(f);return;}
case 7:if(v.count!=2||!v.a[0].n||v.a[0].n>FEATURE_REQUEST_TITLE_MAX||v.a[1].n>FEATURE_REQUEST_DESCRIPTION_MAX||!file_utf8(v.a[0].d,v.a[0].n)||!file_utf8(v.a[1].d,v.a[1].n))break;tool_request(id,v.a[0].d,v.a[0].n,v.a[1].d,v.a[1].n);return;
case 8:if(v.count)break;tool_assets(id);return;
case 9:if(v.count!=3||!v.a[0].n||!file_utf8(v.a[0].d,v.a[0].n)||!canonical(v.a[1])||!canonical(v.a[2]))break;tool_asset_read(id,v.a[0].d,v.a[0].n,v.a[1].d,v.a[1].n,v.a[2].d,v.a[2].n);return;
case 10:{if(v.count!=2||!v.a[0].n||!file_utf8(v.a[0].d,v.a[0].n)||!path(v.a[1]))break;u64 f=lock();int done=asset_import(v.a[0].d,v.a[0].n,v.a[1].d,v.a[1].n);unlock(f);if(!done)break;ok(id);return;}
case 11:{if(v.count!=1||!path(v.a[0]))break;u64 f=lock();int task=task_file(v.a[0].d,v.a[0].n);unlock(f);if(task<0)break;number(id,(u32)task);return;}
case 12:{u32 task;if(v.count!=1||!dec(v.a[0],&task))break;u64 status;int state=task_wait(task,&status);if(state<0)break;if(!state){header(id,0,7);serial_write_bytes((const u8*)"running",7);}else number(id,status);return;}
