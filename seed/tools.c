...
static const struct bytes names[]={N("list"),N("read"),N("write"),N("truncate"),N("remove"),N("build"),N("finish_generation"),N("request_feature"),N("list_provided_assets"),N("read_provided_asset")};
...
static void header(uint32_t id,uint32_t status,uint32_t n){frame_send_header(INVOKE_TOOL_RESPONSE,id,4u+n);frame_write_u32(status);}void tools_send_failure(uint32_t id){header(id,1,0);}static void ok(uint32_t id){header(id,0,0);}
...
case 0:{struct bytes prefix={0,0};if(v.count>1||(v.count==1&&!utf8(v.a[0].d,v.a[0].n)))break;if(v.count)prefix=v.a[0];uint32_t out=0;for(uint32_t i=0;i<file_count;++i)if(file_path_has_prefix(&files[i],prefix.d,prefix.n))out+=files[i].path_length+1u;header(id,0,out);for(uint32_t i=0;i<file_count;++i)if(file_path_has_prefix(&files[i],prefix.d,prefix.n)){serial_write_bytes(files[i].path,files[i].path_length);serial_write('\n');}return;}
case 1:{uint32_t off,len;if(v.count!=3||!path(v.a[0])||!dec(v.a[1],&off)||!dec(v.a[2],&len)||len>FRAME_MAX_PAYLOAD-4u)break;struct file*f=file_find(v.a[0].d,v.a[0].n);if(!f||off>file_size(f))break;uint32_t left=file_size(f)-off,out=len<left?len:left;header(id,0,out);serial_write_bytes(file_content(f)+off,out);return;}
case 2:{uint32_t off;if(v.count!=3||!path(v.a[0])||!dec(v.a[1],&off)||!file_write(v.a[0].d,v.a[0].n,off,v.a[2].d,v.a[2].n))break;ok(id);return;}
case 3:{uint32_t z;if(v.count!=2||!path(v.a[0])||!dec(v.a[1],&z)||!file_truncate(v.a[0].d,v.a[0].n,z))break;ok(id);return;}
case 4:if(v.count!=1||!path(v.a[0])||!file_remove(v.a[0].d,v.a[0].n))break;ok(id);return;
case 5:if(v.count)break;build_tool_invoke(id);return;
case 6:if(v.count!=1||v.a[0].n>FINISH_GENERATION_HANDOFF_MAX||!utf8(v.a[0].d,v.a[0].n))break;finish_generation_tool_invoke(id,v.a[0].d,v.a[0].n);return;
