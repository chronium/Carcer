...
case 12:{uint32_t task;if(v.count!=1||!dec(v.a[0],&task))break;uint64_t status;int state=task_reap(task,&status);if(state<0)break;if(!state){header(id,0,7);serial_write_bytes((const uint8_t*)"running",7);}else number(id,status);return;}
